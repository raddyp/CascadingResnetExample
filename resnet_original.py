...
def model_configuration():
	...
	steps_per_epoch = tf.math.floor(train_size / batch_size)
	val_steps_per_epoch = tf.math.floor(val_size / batch_size)
	epochs = tf.cast(tf.math.floor(maximum_number_iterations / steps_per_epoch),\
		dtype=tf.int64)
...
def residual_block(x, number_of_filters, match_filter_size=False):
	...
	if match_filter_size:
		x = tfl.Conv2D(number_of_filters, kernel_size=(3, 3), strides=(2,2),\
			kernel_initializer=initializer, padding="same")(x_skip)
	else:
		x = tfl.Conv2D(number_of_filters, kernel_size=(3, 3), strides=(1,1),\
			kernel_initializer=initializer, padding="same")(x_skip)
	x =tfl.BatchNormalization(axis=3)(x)
	x = tfl.Activation("relu")(x)
	x = tfl.Conv2D(number_of_filters, kernel_size=(3, 3),\
	...
	if match_filter_size and config.get("shortcut_type") == "identity":
		x_skip = tfl.Lambda(lambda x: tf.pad(x[:, ::2, ::2, :], tf.constant([[0, 0,], [0, 0], [0, 0], [number_of_filters//4, number_of_filters//4]]), mode="CONSTANT"))(x_skip)
	elif match_filter_size and config.get("shortcut_type") == "projection":
		x_skip = tfl.Conv2D(number_of_filters, kernel_size=(1,1),\
...
def model_base(shp):
	...
	inputs = tfl.Input(shape=shp)
	x = tfl.Conv2D(config.get("initial_num_feature_maps"), kernel_size=(3,3),\
	...
	x = tfl.Flatten()(x)
	outputs = tfl.Dense(config.get("num_classes"), kernel_initializer=initializer)(x)
